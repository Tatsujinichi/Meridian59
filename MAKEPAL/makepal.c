 ...
 */
unsigned char GetClosestPaletteIndexU(int red, int blue, int green)
{
...
void MakeLightPalettes(void)
{
   ...
   for (i=0; i < LIGHT_LEVELS; i++)
   {
	 ...
	 light_palettes[i][j] = GetClosestPaletteIndexU(red, blue, green);
      }
   }
   ...
   for (j=0; j < NUM_COLORS; j++)
   {
	 ...
	 continue;
      ...
      light_palettes[PALETTE_INVERT][j] = GetClosestPaletteIndexU(red, blue, green);
   }
}
/************************************************************************/
void MakeBlendBiXlat(unsigned char* pBiXlat, int partsFirst, int partsSecond)
{
	...
	for (i = 0; i < NUM_COLORS; i++)
	{
		...
		for (j = 0; j < NUM_COLORS; j++)
		{
			...
			pBiXlat[(i<<8)|(j)] = GetClosestPaletteIndexU(pe.red, pe.blue, pe.green);
		}
	}
}
